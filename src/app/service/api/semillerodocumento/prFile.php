...
require_once("../database.php");  
 $Id = json_decode($_POST['DSEM_CODI']);
 $response = array();

 if (!file_exists($_SERVER['DOCUMENT_ROOT'] .'/AppInvestigador/DS/')) {
    mkdir($_SERVER['DOCUMENT_ROOT'] .'/AppInvestigador/DS/', 0777, true);
}

 $upload_dir = $_SERVER['DOCUMENT_ROOT'] .'/AppInvestigador/DS/';
...
else 
{
    ...
    if(move_uploaded_file($foto_tmp_name , $upload_name)) {
        $response = array(
          ...
          $SQL="UPDATE  sgi_docu_semi set DSEM_PATH='" . $random_name . "' WHERE DSEM_CODI =" . $Id;
